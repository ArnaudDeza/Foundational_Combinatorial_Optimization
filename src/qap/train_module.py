...
class QAPModule(DOP_Module):
    """
    QAP-specific implementation of DOP_Module
    """
    
    def _step(self, batch, stage=None, dataloader_idx=None):
        """
        QAP-specific logic for training/validation/test steps.

        Args:
            batch: A tuple containing QAP data - either:
                   (F_batch, D_batch, sol_batch, mask_batch, hash_id_batch, obj_batch) for supervised
                   (F_batch, D_batch, mask_batch, hash_id_batch) for unsupervised
        ...
        batch_len = len(batch)
        if batch_len == 4:
            F, D, mask, hash_ids = batch 
            have_opt_sols = False
        else:
            F, D, opt_sols, mask, hash_ids, opt_obj = batch
            have_opt_sols = True
 
        batch_size = F.shape[0]

        if self.learning == "supervised":
            loss, metrics = self.model.supervised(F, D, mask, opt_sols)
        elif self.learning == "unsupervised_obj": 
            loss, metrics = self.model.unsupervised_obj(F, D, mask)
            ...
            loss, metrics, updated_cache_sols, updated_cache_objs = self.model.gst_step(F, D, mask, cache_sols, cache_objs)
            # Update the cache with the new solutions and objectives 
            if stage in ["val","test"] and dataloader_idx > 0:
                for i, hash_id in enumerate(hash_ids): 
                    self.IMPROVE_CACHE_rw[hash_id] = [updated_cache_sols[i], updated_cache_objs[i]]
            else:
                for i, hash_id in enumerate(hash_ids): 
                    self.IMPROVE_CACHE[hash_id] = [updated_cache_sols[i], updated_cache_objs[i]]
